...
void * free_list[EASYJSONPARSER_MAX_ALLOC_TRACK];
...
{
  if (free_list_counter >= EASYJSONPARSER_MAX_ALLOC_TRACK) {
    fprintf(stderr, "cannot build test suite, exceeds suite memory tracking, increase EASYJSONPARSER_MAX_ALLOC_TRACK or fix issue");
    exit(1);
  }
...
char ** new_tags ()
{
  char ** tags = (char **) tracked_malloc(sizeof(char *) * EASYJSONPARSER_MAX_REC_LEVELS);

  for (int i = 0; i < EASYJSONPARSER_MAX_REC_LEVELS; i++)
    tags[i] = NULL;
...
{
  char ** new_tags = (char **) tracked_malloc(sizeof(char *) * EASYJSONPARSER_MAX_REC_LEVELS);
  memcpy(new_tags, tags, sizeof(char *) * EASYJSONPARSER_MAX_REC_LEVELS);
...
void * new_fixtures ()
{
  void (**fixtures)() = (void (**)()) tracked_malloc(sizeof(void (*)()) * EASYJSONPARSER_MAX_REC_LEVELS * 2);

  for (int i = 0; i < EASYJSONPARSER_MAX_REC_LEVELS * 2; i++)
    fixtures[i] = NULL;
...
{
  void (**new_fixtures)() = (void (**)()) tracked_malloc(sizeof(void (*)()) * EASYJSONPARSER_MAX_REC_LEVELS * 2);
  memcpy(new_fixtures, fixtures, sizeof(char *) * EASYJSONPARSER_MAX_REC_LEVELS);

  for (int i = 0; i < EASYJSONPARSER_MAX_REC_LEVELS * 2; i += 2)
...
{
  char tag_str[EASYJSONPARSER_MAX_REC_LEVELS * 2];
